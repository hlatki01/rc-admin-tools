    ...
    async function getDepartments(status) {
      ...
      if (data.departments.length > 0) {
        data.departments.forEach(element => {
          if (status === 'all') {
            $('#loading').show();
            dataSet.push({ id: element._id, enabled: element.enabled, name: element.name, email: element.email, _updatedAt: element._updatedAt, showOnRegistration: element.showOnRegistration, showOnOfflineForm: element.showOnOfflineForm })
            $('#loading').hide();
          }
    ...
    async function createNewTable() {
      ...
      var table = new Tabulator("#table", {
        ...
        columns: [ //Define Table Columns
          ...
          { title: "showOnRegistration", field: "showOnRegistration" },
          { title: "showOnOfflineForm", field: "showOnOfflineForm" },
      ...
      document.getElementById("disable-rows").addEventListener("click", async function () {
        var selectedRows = table.getSelectedData(); //get array of currently selected row components.
        let btn = await sendConfirmation(`Do you want to really want to modify these ${selectedRows.length} registers?`)
        if (btn.isConfirmed) {
      ...
      document.getElementById("enable-rows").addEventListener("click", async function () {
        var selectedRows = table.getSelectedData(); //get array of currently selected row components.
        let btn = await sendConfirmation(`Do you want to really want to modify these ${selectedRows.length} registers?`)
        if (btn.isConfirmed) {
      ...
      document.getElementById("delete-disabled-rows").addEventListener("click", async function () {
        ...
        let btn = await sendConfirmation(`Do you want to really want to delete these ${disabledRows.length} registers?`)
        if (btn.isConfirmed) {
